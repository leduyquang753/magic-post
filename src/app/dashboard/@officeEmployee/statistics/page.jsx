...
async function getStatistics([url, timeRangeKey]) {
	...
	return {
		...
		datasets: [
			{label: "Thành công", data: data.map(point => point.status.success)},
			{label: "Thất bại", data: data.map(point => point.status.fail)}
		]
	};
}
...
export default function OfficeStatisticsPage() {
	...
	const {data, error, isLoading, mutate} = useSWR(
		[`${apiUrl}/statistic/orderStatus?`, timeRange], getStatistics
	);
	
	return <>
		<DashboardPageTitle>Thống kê điểm giao dịch</DashboardPageTitle>
		...
		<Card>
			<CardHeader size="large" header={<Title2>Giao hàng</Title2>}/>
			{
