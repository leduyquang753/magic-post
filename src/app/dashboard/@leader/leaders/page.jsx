...
async function getAccounts(url) {
	const response = await fetch(url + "?" + new URLSearchParams({role: "leader"}), {credentials: "include"});
	return await response.json();
}

export default function LeaderAccountsPage() {
	useEffect(() => { document.title = "Tài khoản lãnh đạo | Magic Post"; }, [true]);
	...
	const resetFormData = () => {
		...
		setEditingUsername("");
		setEditingFullName(""); setEditingPassword("");
	...
	const columnDefinitions = [
		...
		createTableColumn({
			...
			renderCell: account => <>
				<Tooltip relationship="label" content="Sửa thống tin"><Button
				...
				/></Tooltip>
				<Tooltip relationship="label" content="Đổi mật khẩu"><Button
				...
				/></Tooltip>
				{accountStatus.username !== account.username && <Tooltip relationship="label" content="Xóa">
					<Button
	...
	return <>
		<DashboardPageTitle>Các tài khoản lãnh đạo</DashboardPageTitle>
		<Card>
			<div className="flex flex-row">
				<div className="flex-1">
					...
					>Thêm tài khoản lãnh đạo</Button>
				</div>
				<div><Tooltip relationship="label" content="Refresh results">
		...
		<Dialog open={isCreationDialogOpen} onOpenChange={(_, {open}) => {
			...
			<DialogSurface><DialogBody>
				<DialogTitle>Tạo tài khoản lãnh đạo</DialogTitle>
				<DialogContent>
		...
		<Dialog open={isEditingDialogOpen} onOpenChange={(_, {open}) => {
			...
			<DialogSurface><DialogBody>
				<DialogTitle>Sửa thông tin tài khoản lãnh đạo</DialogTitle>
				<DialogContent>
					{isOperationFailed && <Info
						icon={<ErrorCircle20Filled/>}
						backgroundColor={tokens.colorStatusDangerBackground2}
		...
		<Dialog open={isPasswordChangeDialogOpen} onOpenChange={(_, {open}) => {
			...
			<DialogSurface><DialogBody>
				<DialogTitle>Đổi mật khẩu tài khoản lãnh đạo</DialogTitle>
				<DialogContent>
					{isOperationFailed && <Info
						icon={<ErrorCircle20Filled/>}
						backgroundColor={tokens.colorStatusDangerBackground2}
		...
		<Dialog open={isDeletionDialogOpen} onOpenChange={(_, {open}) => {
			...
			<DialogSurface><DialogBody>
				<DialogTitle>Xóa tài khoản lãnh đạo?</DialogTitle>
				<DialogContent>
					{isOperationFailed && <Info
						icon={<ErrorCircle20Filled/>}
						backgroundColor={tokens.colorStatusDangerBackground2}
						foregroundColor={tokens.colorStatusDangerForeground2}
					>Thao tác đã bị thất bại.</Info>}
					<Body1>Bạn có chắc muốn xóa tài khoản lãnh đạo "{editingUsername}"?</Body1>
				</DialogContent>
				<DialogActions fluid>
					{isOperationInProgress && <Spinner size="tiny" className="mr-2"/>}
					<DialogTrigger disableButtonEnhancement>
